...
export default function StressView() {
  ...
  const sumaKosztowLab = isZKrwi ? labSnapshot.sumaKosztow : 0;
  const sumaCenLab = isZKrwi ? labSnapshot.sumaCen : 0;
  const sumaRynkowaLab = isZKrwi ? labSnapshot.sumaRynkowa : 0;
  ...
  const logistykaFull =
    logistykaBase +
    (isZKrwi && labSnapshot.szczegolyPakietow.trim()
      ? `\n**Skład wybranego laboratorium:**\n${labSnapshot.szczegolyPakietow}\n`
      : "");

  const labKonfigOk = !isZKrwi || labSnapshot.konfiguracjaPoprawna;

  const handleAddToCart = () => {
    const detal = isZKrwi ? labSnapshot.labCartDetal : null;
    addToCart({
      usluga: "Zarządzanie stresem",
      cenaBrutto: finalPrice,
      cenaPerCapita,
      cenaRynkowaOsoba: sumaRynkowaLab,
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="flex flex-col gap-3">
            <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-200 p-4 transition-colors hover:bg-slate-50">
              ...
              />
              <span className="force-wrap text-slate-700">
                {VARIANT_BEZ_KRWI}
              </span>
            </label>
            <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-200 p-4 transition-colors hover:bg-slate-50">
      ...
      {isZKrwi && (
        <Card className="shadow-sm transition-all duration-200">
          <CardHeader>
            <CardTitle>Laboratorium (jak w module Lab)</CardTitle>
            <p className="text-sm font-normal text-slate-600">
              Wybierz pakiet bazowy lub Custom, dodaj badania pojedyncze —
              koszt krwi / osoba sumuje się automatycznie.
            </p>
          </CardHeader>
          <CardContent>
              ...
              return (
                ...
                >
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      <Label>
                        {isZKrwi ? "Liczba Zespołów Lab" : "Liczba Zespołów"}
                      </Label>
                      <Input
                        type="number"
                        min={1}
                  ...
                  {patients > 0 && isZKrwi && (
                    <div className="mt-4 rounded-lg border-l-4 border-blue-400 bg-blue-50/50 px-4 py-2 text-sm text-slate-700">
                      Logistyka Lab: {loc.teams} zesp. — <strong>{dni} dni</strong>{" "}
                      pracy.
                    </div>
                  )}
                </div>
